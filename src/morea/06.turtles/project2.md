...
***Every submitted picture should look unique.***### star_polygon.pyWe can use a similar technique to draw stars with an arbitrary number of points. Write a program `star_polygon.py` with the following function:`star_polygon(num_pts, num_skip, side_length)`: takes the number of points (`num_pts`), the number of points to skip to calculate the next angle (`num_skip`)  and the length of each line in the star (`side_length`) as parameters. <!--You will need a different approach to drawing the stars depending on if there is an odd or even number of points.-->
